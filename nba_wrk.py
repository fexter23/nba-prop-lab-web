...
if selected_player:
    ...
    with cols[1]:
        line_val = st.selectbox("Line", dropdown_values(), 
    ...
    with cols[4]:
        def get_board_json():
            data = [entry.copy() for entry in st.session_state.my_board]
            for item in data:
...
# ── Pin Logic (NOW AFTER df is loaded) ─────────────────────────────────────────
if pin_clicked and selected_player and selected_stat and selected_stat != "— Select stat —" and \
   selected_stat in lines and df is not None and not df.empty:
...
if lines:
    ...
    for stat, line in lines.items():
        ...
        window_labels = [f"L{w}" for w in windows]
        parts = [f"<span style='color:{'#00ff88' if pct > 73 else '#ffcc00' if pct >= 60 else '#ff5555'}'>{pct:.0f}%</span> ({lbl})" 
                for pct, lbl in zip(over_list, window_labels)]
        ...
        st.markdown(f"<div style='background:#1e1e2e;padding:12px;border-radius:8px;margin:8px 0;'><strong>{stat} {line}</strong> {hit_str} AVG: <span style='color:{avg_color_o}'>O {avg_o:.0f}%</span> / <span style='color:{avg_color_u}'>U {avg_u:.0f}%</span></div>", unsafe_allow_html=True)
        ...
        s_count = sum(1 for r in res_main if r == res_main[0])
        st.markdown(f"**Current Streak: {s_type}{s_count}**")

        fig = go.Figure()
        colors = ["#00ff88" if v > line else "#ff4444" for v in recent_data[stat]]
        fig.add_trace(go.Bar(x=recent_data["GAME_DATE"], y=recent_data[stat], marker_color=colors,
                            text=recent_data[stat].round(1), textposition="inside"))
        fig.add_hline(y=line, line_dash="dash", line_color="#00ffff", annotation_text=f"line = {line}")
        fig.update_layout(height=320, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#00e0ff")
        st.plotly_chart(fig, use_container_width=True)

# Minutes Projection & Game Log (same as before)
...
if len(recent_min) >= 3:
    ...
    concern = "🟢 Solid" if projected_min >= 32 else "🟡 Some concern" if projected_min >= 28 else "🔴 High risk"
    arrow = "↑" if slope_min > 0.3 else "↓" if slope_min < -0.3 else "→"
    st.markdown(f"**Projected Minutes**: ≈ **{projected_min:.1f}** | **Avg (L10)**: **{recent_avg_min:.1f}** ({arrow} {slope_min:.1f}) — **{concern}**")
...
with st.expander("📊 Full Recent Game Log (Last 15)", expanded=False):
    ...
    st.dataframe(df.head(15)[available_cols].style.format(precision=1), use_container_width=True, hide_index=True)
