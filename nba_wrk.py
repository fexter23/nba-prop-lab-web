...
pid = get_player_id(selected_player) if selected_player else None   # defined below
player_team = player_team_map.get(str(pid), "???") if pid else "???"

# ── Stat, Line & Odds on one line ───────────────────────────────────────────────
...
if selected_player:
    stat_cols = st.sidebar.columns([1.8, 1.5, 1.7])
    ...
    with stat_cols[1]:
        line_val = st.selectbox(
            ...
            key=f"line_{selected_stat if selected_stat != '— Select stat —' else 'none'}", 
            label_visibility="collapsed"
        )
...
# ── Pin Button ──────────────────────────────────────────────────────────────────
if selected_stat and selected_stat != "— Select stat —" and lines and st.sidebar.button("📌 Pin to Board", use_container_width=True):
...
# ── My Dashboard, Download, Upload, etc. (unchanged) ─────────────────────────────
# ... [The rest of your original code remains the same from here] ...
...
# Download / Upload Board (unchanged)
def get_board_json():
    data = []
...
# ── Main content (unchanged from here) ──────────────────────────────────────────
...
# Hit rate display, charts, game log, etc.
if lines:
    pdata = df.sort_values("GAME_DATE_DT", ascending=False).copy()
    ...
    for stat, line in lines.items():
        ...
        # Bar chart and rest of the original main content...
        if len(pdata) > 0:
            n = min(10, len(pdata))
            ...
            fig.add_hline(y=line, line_dash="dash", line_color="#00ffff", annotation_text=f"line = {line}", annotation_position="top right")
...
# Minutes projection and game log table remain the same...
# (Copy the rest of your original main content here if needed)
